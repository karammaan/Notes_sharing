...
$password = $data_decoded['pass_key'];
...
$result = mysqli_query($connection , "select Email_id from user where Email_id = '$email' ");
...
mysqli_query($connection, "insert into user (Username , Email_id , Mobile_no , Password) 
values ('$name' , '$email' , '$mobile' , '$password') " );

	// if data is inserted we will create response by making key value pair
	// here key name is 'key' and value is 1
	
	$result = mysqli_query($connection , "select User_id from user where Email_id = '$email' ");
	
	$fetched_row = mysqli_fetch_array($result);
	
	$response['user_id'] = $fetched_row['User_id'] ;
